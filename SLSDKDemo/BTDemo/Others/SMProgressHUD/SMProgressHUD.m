...
#define SMMainThreadAssert() NSAssert([NSThread isMainThread], @"SMProgressHUD needs to be accessed on the main thread.");

CGFloat const SMProgressMaxOffset = 1000000.f;

static const CGFloat SMDefaultPadding = 4.f;
static const CGFloat SMDefaultLabelFontSize = 16.f;
static const CGFloat SMDefaultDetailsLabelFontSize = 12.f;
...
@interface SMProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    SMProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    SMProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (SMProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (SMProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = SMProgressHUDAnimationFade;
    _mode = SMProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(SMProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == SMProgressHUDAnimationZoom) {
        type = animatingIn ? SMProgressHUDAnimationZoomIn : SMProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == SMProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == SMProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == SMProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == SMProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    SMProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<SMProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    SMBackgroundView *backgroundView = [[SMBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = SMProgressHUDBackgroundStyleSolidColor;
    ...
    SMBackgroundView *bezelView = [SMBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:SMDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:SMDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [SMProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:SMDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[SMRoundProgressView class]];

    SMProgressHUDMode mode = self.mode;
    if (mode == SMProgressHUDModeIndeterminate) {
    ...
    else if (mode == SMProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[SMBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == SMProgressHUDModeDeterminate || mode == SMProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[SMRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == SMProgressHUDModeAnnularDeterminate) {
            [(SMRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == SMProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == SMProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[SMProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[SMProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[SMRoundProgressView class]]) {
        SMRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [SMRoundProgressView appearanceWhenContainedIn:[SMProgressHUD class], nil];
#else
        appearance = [SMRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[SMProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((SMRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((SMRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[SMBarProgressView class]]) {
        SMBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [SMBarProgressView appearanceWhenContainedIn:[SMProgressHUD class], nil];
#else
        appearance = [SMBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[SMProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((SMBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((SMBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    SMBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? SMDefaultPadding : 0.f;
...
- (void)setMode:(SMProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == SMProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation SMRoundProgressView
...
@implementation SMBarProgressView
...
@implementation SMBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = SMProgressHUDBackgroundStyleBlur;
            ...
            _style = SMProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(SMProgressHUDBackgroundStyle)style {
    if (style == SMProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = SMProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    SMProgressHUDBackgroundStyle style = self.style;
    if (style == SMProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == SMProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation SMProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [SMProgressHUD allHUDsForView:view];
    for (SMProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable SMProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    SMBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == SMProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    SMMainThreadAssert();
    self.backgroundView.style = SMProgressHUDBackgroundStyleSolidColor;
...
@implementation SMProgressHUDRoundedButton
...
//    self.layer.cornerRadius = ceil(height / 5.f);
}
