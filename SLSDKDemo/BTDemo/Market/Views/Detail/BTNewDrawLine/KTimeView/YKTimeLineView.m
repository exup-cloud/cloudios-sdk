...
@property (nonatomic, assign) NSInteger timeLineDrawNum;
...
- (UIPinchGestureRecognizer *)pinchGesture {
    if (!_pinchGesture) {
        _pinchGesture = [[UIPinchGestureRecognizer alloc] initWithTarget:self action:@selector(handlePinchGestureAction:)];
    }
    return _pinchGesture;
}

- (void)handlePinchGestureAction:(UIPinchGestureRecognizer *)recognizer {
    CGFloat scale = recognizer.scale - self.lastPanScale + 1;
    
    //    [self postNotificationWithGestureRecognizerStatus:recognizer.state];
    ...
    CGFloat forwardDrawCount = self.timeLineDrawNum;
    
    _timeLineDrawNum = floor((self.frame.size.width - self.contentLeft - self.contentLeft) / (self.volumeWidth));
    
    //容差处理
    CGFloat diffWidth = (self.frame.size.width - self.contentLeft - self.contentLeft) - (self.volumeWidth)*_timeLineDrawNum;
    if (diffWidth > 4*(self.volumeWidth)/5.0) {
        _timeLineDrawNum = _timeLineDrawNum + 1;
    }
    
    //    _kLineDrawNum = self.contexts.count > 0 && _kLineDrawNum < self.contexts.count ? _kLineDrawNum : self.contexts.count;
    if (forwardDrawCount == self.timeLineDrawNum) {
        return;
    }
    
    NSInteger diffCount = fabs(self.timeLineDrawNum - forwardDrawCount);
    ...
    //    self.startDrawIndex = self.startDrawIndex + self.timeLineDrawNum > self.contexts.count ? self.contexts.count - self.kLineDrawNum : self.startDrawIndex;
    
    [self resetMaxAndMin];
    
    recognizer.scale = scale;
    self.lastPanScale = recognizer.scale;
