...
extension CHDepthChartView {
    ...
    fileprivate func drawYAxisLabel(_ yAxisToDraw: [(CGRect, String)]) {
        ...
        for (yLabelRect, strValue) in yAxisToDraw {
            ...
            yAxisLabel.string = BasicParameter.dealDataFormate(strValue)
    ...
    fileprivate func drawXAxisLabel(xAxisToDraw: [(CGRect, String)]) {
        ...
        //原始为self.bounds.maxY,不明白为啥要画到外面
        
        let startY = self.bounds.maxY - self.padding.bottom
        //需要显示x坐标标签名字的分区，再最下方显示
    ...
    func drawItemInfoX(point: CGPoint,item:CHKDepthChartItem){
        ...
        selectedItemXLayer?.backgroundColor = UIColor.ThemeView.bg.cgColor
        selectedItemXLayer?.borderWidth = 0.5
        selectedItemXLayer?.borderColor = UIColor.ThemeView.seperator.cgColor
...
//            .ch_toString(maxF:self.decimal)//String(Double(iteme.value))
        ...
        pricelayer.alignmentMode = kCAAlignmentCenter
    ...
    func drawItemInfoY(point: CGPoint,item:CHKDepthChartItem){
        ...
        let textWidth = BasicParameter.dealVolumFormate("\(item.depthAmount)") .textHeightSizeWithFont(UIFont.ThemeFont.MinimumRegular, height: 10).width
        let width : CGFloat = textWidth + 20
        let height : CGFloat = 20
        ...
        selectedItemYLayer?.backgroundColor = UIColor.ThemeView.bg.cgColor
        selectedItemYLayer?.borderWidth = 0.5
        selectedItemYLayer?.borderColor = UIColor.ThemeView.seperator.cgColor
        
        // 价格
        let amountlayer = CHTextLayer()
        amountlayer.string = BasicParameter.dealVolumFormate("\(item.depthAmount)")
        //            .ch_toString(maxF:self.decimal)//String(Double(iteme.value))
        amountlayer.frame = CGRect(x: 10, y: 5, width: textWidth, height: 10)
        amountlayer.alignmentMode = kCAAlignmentCenter
    ...
    func drawItemInfo(point: CGPoint,item:CHKDepthChartItem){
        ...
        if let view = self.delegate?.depthChartShowItemView?(chart: self, Selected: item) {
            ...
            typelayer.foregroundColor =  UIColor.ThemeView.bg.cgColor
            ...
            pricelayer.foregroundColor =  UIColor.ThemeView.bg.cgColor
            pricelayer.backgroundColor = UIColor.clear.cgColor
            pricelayer.contentsScale = UIScreen.main.scale
            ...
            vollayer.foregroundColor =  UIColor.ThemeView.bg.cgColor
