...
open class CHChartModel {
    ...
    open var titleColor = UIColor.ThemeView.bg                  //标题文本的颜色
    ...
    //修改为1.5
    open var lineWidth: CGFloat = 0.5                                     //线段宽度
...
//        let maxScrennWidth = SCREEN_WIDTH - rightOffset
...
//            let ix = self.section.frame.origin.x + self.section.padding.left + CGFloat(i - startIndex) * plotWidth
            ...
            if i == datas.count - 1,self.key  == "Timeline_Timeline"{
                ...
                pointLayer.backgroundColor = UIColor.ThemeView.highlight.cgColor
        ...
        if self.key  == "Timeline_Timeline" {
            ...
            gradientLayer.colors = [UIColor.ThemeView.highlight50.cgColor, UIColor.clear.cgColor]
...
public extension CHChartModel {
    ...
    public func drawGuideValue(value: String, section: CHSection, point: CGPoint, trend: CHChartItemTrend) -> CAShapeLayer {
        ...
        var guideValueTextColor: UIColor = UIColor.ThemeView.bg              //显示最大最小的文字颜色
        //判断绘画完整时是否超过界限
...
//            let textSize = CGSize(width: SCREEN_WIDTH - maxPriceStartX, height: 12)
            //画最大值数字
            let valueText = CHTextLayer()
            //原为point，现改为+高度,哪块没改对，todo当前y
