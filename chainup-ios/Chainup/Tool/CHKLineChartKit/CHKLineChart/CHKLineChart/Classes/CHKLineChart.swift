...
open class CHKLineChartView: UIView {
    ...
    public let kYAxisLabelWidth: CGFloat = 70        //默认宽度
    ...
    open var selectedPosition: CHChartSelectedPosition = .free         //选中显示y值的位置
...
//            self.sightView?.isHidden = !self.showSelection
    ...
    open var range: Int = 40                             //显示在可见区域的个数
    var borderColor: UIColor = UIColor.gray
    open var labelSize = CGSize(width: 40, height: 16)
    
    var datas: [CHChartItem] = [CHChartItem]()      //数据源
    
    open var selectedBGColor: UIColor = UIColor.ThemeLabel.colorMedium   //选中点的显示的框背景颜色
...
//    }
    ...
    fileprivate func initUI() {
        ...
        self.horizontalLineView?.backgroundColor = UIColor.ThemeLabel.colorMedium
...
//                self.horizontalLineView?.frame = CGRect(x: hx - plotWidth/2, y: hy, width: plotWidth, height: hheight)
                self.horizontalLineView?.frame = CGRect(x: hx - 2, y: hy, width: 4, height:hheight - 30)
                ...
                self.selectedYAxisLabel?.indexLabel.text = String(format: format, yVal)     //显示实际值
...
//                    self.rangeTo = self.plotCount - 1             //默认是数据最后一条为尽头
//                    if plotCount > 36 {
...
//        #if DEBUG
//        print("\n一共\(self.plotCount)根蜡烛,\n绘制第\(self.rangeFrom)到\(self.rangeTo),\n范围\(self.range),\n滑动的趋势是\(self.scrollToPosition)")
//        #endif
...
//        print("========== Draw Section")
        ...
        borderLayer.strokeColor = UIColor.ThemeView.seperator.cgColor
...
//                referenceLayer.lineDashPattern = pattern
            ...
            if showYAxisLabel {
                ...
                let labelY = (yVal == maxValue) ? startY + 10 : startY - 10
    ...
    func drawLogo(_ section: CHSection) {
        ...
        if let imgUrl = URL.init(string: imgStr) {
            ...
            if EXHomeViewModel.homepageStyle() == .king{
                logoLayer.yy_setImage(with: nil, placeholder: UIImage.themeImageNamed(imageName: "king_logo"))
            }else {
            ...
            logoLayer.contentsGravity = kCAGravityTopLeft
...
//        self.drawLayerView()
    ...
    public func zoomChart(by interval: Int, enlarge: Bool) {
        print("出发scale\(interval),行为:\(enlarge)")
...
class HorizontalXLineView:UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let colors = [UIColor.white.cgColor,UIColor.ThemeView.bgIcon25.cgColor]
...
class SelectedYAxisLabel :UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        path .close()
        UIColor.ThemeView.seperator.setStroke()
