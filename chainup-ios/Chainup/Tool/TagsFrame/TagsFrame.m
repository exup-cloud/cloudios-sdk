...
- (void)setTagsArray:(NSArray *)tagsArray
{
    ...
    for (NSInteger i=0; i<tagsArray.count; i++) {
        ...
        btnW = [self sizeWithText:tagsArray[i] font:TagsTitleFont].width + _tagsMinPadding;
        
        if (i < tagsArray.count-1) {
            nextWidth = [self sizeWithText:tagsArray[i+1] font:TagsTitleFont].width + _tagsMinPadding;
        }
    ...
    for (NSInteger i = 0; i < lastIndexs.count; i++) {
        ...
        CGFloat tagX = 0;
        CGFloat tagY = 0 + (_tagsLineSpacing + tagH) * i;
    
        for (NSInteger j = 0; j < newArr.count; j++) {
            
            tagW = [self sizeWithText:newArr[j] font:TagsTitleFont].width + _tagsMinPadding + averageW;
